    ...
    module Text
      def self.included(base)
        base.class_eval do
          ...
          def p(*args, &block)
            ...
            if model.valid?
              ...
              raise Caracal::Errors::InvalidModelError,
                    'Paragraphs and headings, which delegate to the :p command, require at least one text string.'
            end
            model
          end
          ...
          #
          %i[h1 h2 h3 h4 h5 h6].each do |cmd|
            define_method cmd.to_s do |*args, &block|
              options = Caracal::Utilities.extract_options!(args)
